...
interface Props {
  openPositions:   SpotPosition[]
  closedPositions: SpotPosition[]
}
...
export function SpotPositionsTable({ openPositions, closedPositions }: Props) {
  ...
  function pnlCell(pnl: number | undefined) {
    ...
    const sign  = pnl >= 0 ? '+' : ''
    return <span className={color}>{sign}{pnl.toFixed(4)} SOL</span>
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-zinc-500 text-xs uppercase border-b border-zinc-800">
                ...
                <th className="text-right px-4 py-3">TP / SL</th>
              ...
              {rows.map(pos => (
                <tr key={pos.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-zinc-500 text-xs">{pos.mint.slice(0, 8)}...</div>
                  </td>
                  ...
                  <td className="text-right px-4 py-3 text-zinc-400 text-xs">
                    +{pos.tp_pct}% / {pos.sl_pct}%
                  </td>
                  ...
                  <td className="text-center px-4 py-3">
                    {pos.tx_buy && (
                      <a
                        href={`https://solscan.io/tx/${pos.tx_buy}`}
                        target="_blank"
                        rel="noreferrer"
                    ...
                    {pos.tx_sell && (
                      <>
                        {' '}
                        <a
                          href={`https://solscan.io/tx/${pos.tx_sell}`}
                          target="_blank"
                          rel="noreferrer"
